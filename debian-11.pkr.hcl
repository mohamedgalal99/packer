...
locals {
  ...
  new_img_name_debian11 = "Debian-11"
...
##    extra_arguments = [
##      "--extra-vars", "image=${local.new_img_name_debian11}"
##    ]
##  }
##
##  provisioner "shell" {
...
provisioner "shell" {
  inline = [
    ...
    # Set hostname
    "echo 'debian-11' | sudo tee /etc/hostname",
    "echo '127.0.0.1 debian-11' | sudo tee -a /etc/hosts",

    # Test DNS resolution
    "echo 'Testing DNS resolution...'",
    "getent hosts deb.debian.org || true",
    # Disable stale Debian 11 backports repos in inherited base images
    "if [ -f /etc/apt/sources.list ] && grep -q 'bullseye-backports' /etc/apt/sources.list; then sudo sed -i '/bullseye-backports/s|^|# disabled-by-packer |' /etc/apt/sources.list && echo 'Disabled bullseye-backports in /etc/apt/sources.list'; fi",
    "for f in /etc/apt/sources.list.d/*; do [ -f \"$f\" ] || continue; if grep -q 'bullseye-backports' \"$f\"; then sudo mv \"$f\" \"$f.disabled-by-packer\" && echo \"Disabled bullseye-backports file: $f\"; fi; done",
